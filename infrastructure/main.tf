...
resource "azurerm_virtual_machine_extension" "web_user_data_script" {
  ...
  settings = <<SETTINGS
  {
    "commandToExecute": "git clone https://github.com/yovelchen/Terraform.git && bash /var/lib/waagent/custom-script/download/0/Terraform/infrastructure/scripts/web-user-data.sh"
  }
  SETTINGS
  depends_on = [
...
resource "azurerm_virtual_machine_extension" "db_user_data_script" {
  ...
  settings = <<SETTINGS
  {
    "commandToExecute": "git clone https://github.com/yovelchen/Terraform.git && bash /var/lib/waagent/custom-script/download/0/Terraform/infrastructure/scripts/db-user-data.sh"
  }
  SETTINGS
  depends_on = [
    azurerm_virtual_machine_data_disk_attachment.disk_attachment,
